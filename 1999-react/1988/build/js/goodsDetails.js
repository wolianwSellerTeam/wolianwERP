...
function getSkuList(skuList, skuMap, unitName){
	...
	if(skulength==1){
		...
		for(var i in skuMap){
			...
			str  += '<td><span class="pr_title">￥'+skuMap[i].vendorPrice+'</span></td>';
...
function setSKU1(obj){
	...
	for(var i in goodsDatadetail){
		...
		if(sku1value==objvalue){
			...
			str  += '<td><span class="pr_title">￥'+goodsDatadetail[i].vendorPrice+'<span></td>';
...
function getPriceRange(ladderPriceList, unitName){
	...
	for(var i =0 ; i<ladderLength; i++ ){
			//拼接价格
		   jgtd += '<td class="orange"><i>￥</i><em>'+ladderPriceList[i].vendorPrice+'</em></td>';
		   qjtd += '<td>'+ladderPriceList[i].minNum + unitName+'</td>';
	}

	document.getElementById("vendorPrice").innerHTML = jgtd;
...
function buyNow(){ //立即购买
	...
	layui.use(['element', "woLianw", "layer"], function () {
		...
		if(buyCount<minNum){layer.msg("商品数量必须大于0，并且不能小于最小购买量。",{time:2000});return ;}
		if(maxNum && buyCount>maxNum){layer.msg("商品数量必须大于0，并且不能大于最大购买量。",{time:2000});return ;}
	...
	woLianw.getHotSell(parameter, function(data){
		...
		if(datas.length < 1 ){
			...
			for(var i =0; len = datas.length, i < len; i++){  //// firstThree
				str += '<li>';
					str += '<a href="productDetail.html?productId='+datas[i].productId+'" target="_blank">';
