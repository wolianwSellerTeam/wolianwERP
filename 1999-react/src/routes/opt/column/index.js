...
const pagespace = 'optColumn';
const basemodel = 'opt/column';
const detailname = 'detail';
const tablefetch = 'fetchList';
const searchPlaceholder = '请输入搜索关键字...';

class OptColumn extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    return (
      <FormTablePage
        ...
        headerOperates={(permissionObj && permissionObj.add) ? <div><a href={`/${locale}/${basemodel}/${detailname}?frommenu=${path}`} rel="noopener noreferrer" target="_blank">新增专栏</a></div> : null}
      />
    );
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(OptColumn);
