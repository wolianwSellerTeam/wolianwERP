	...
	var c = {
		...
		createTabDom: function() {
			...
			if(i._parentElem = e.elem, !i.tabDomExists()) {
				var a = ['<div class="layui-tab layui-tab-card kit-tab" lay-filter="' + i._filter + '">', '<ul class="layui-tab-title">', '<li class="layui-this" lay-id="-1"><i class="layui-icon">&#xe68e;</i> 首页</li>', "</ul>", '<ul class="layui-nav layui-layout-right kit-nav">','<li class="layui-nav-item">', '<a href="javascript:;"><img src="http://m.zhengjinfan.cn/images/0.jpg" class="layui-nav-img"><span class="layui-elip">aslihui<span></a>', '<dl class="layui-nav-child">', '<dd>', '<a href="javascript:;"><i class="fa fa-user-circle" aria-hidden="true"></i>&emsp;基本资料</a>', '</dd>', '<dd>', '<a href="javascript:;"><i class="fa fa-gear" aria-hidden="true"></i>&emsp;安全设置</a>', '</dd>', '<dd>', '<a href="javascript:;">&nbsp;<i class="fa fa-sign-out" aria-hidden="true">&nbsp;</i>&emsp;注&emsp;销&emsp;</a>', '</dd>', '</dl>', '</li>', '</ul>','<div class="kit-tab-tool">操作&nbsp;<i class="fa fa-caret-down"></i></div>', '<div class="kit-tab-tool-body layui-anim layui-anim-upbit">', "<ul>", '<li class="kit-item" data-target="refresh">刷新当前选项卡</li>', '<li class="kit-line"></li>', '<li class="kit-item" data-target="closeCurrent">关闭当前选项卡</li>', '<li class="kit-item" data-target="closeOther">关闭其他选项卡</li>', '<li class="kit-line"></li>', '<li class="kit-item" data-target="closeAll">关闭所有选项卡</li>', "</ul>", "</div>", '<div class="layui-tab-content">', '<div class="layui-tab-item layui-show" lay-item-id="-1"><iframe src="' + e.mainUrl + '"></iframe></div>', "</div>", "</div>"];
		...
		winResize: function() {
			...
			/* 检测页面resize 根据div大小  切换大小logo  start******************************************************/
				var $div=t("div.indexLogo");
				if($div.width()<150){
			...
			/* 检测页面resize 根据div大小   切换大小logo  end******************************************************/
